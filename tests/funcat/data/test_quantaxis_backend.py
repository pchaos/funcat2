...
class TestQuantaxisDataBackend(unittest.TestCase):
    ...
    def test_stock_basics(self):
        ...
        self.assertTrue(len(data) > 5000,
                        f"股票、代码数量：{len(data)},实际数量应该大于5000只。")
        self.assertTrue(len(data) > 7000,
                        f"股票、代码数量：{len(data)},实际数量应该大于7000只。")
    ...
    def test_stock_basics2(self):
        ...
        blockname = "科创板50"
        self.assertTrue(blockname in data.values(),
                        f"{blockname} not in stock_basics")
    ...
    def test_symbol(self):
        ...
        for i in range(1, len(codes)):
            self.assertTrue(symbol(codes[i - 1]) !=
                            symbol(codes[i]), "不同代码的名称应该不同")

    def test_get_price(self):
        data = self.qdb.get_price("000001", 20210101, 20210201, '1d')
        self.assertTrue(len(data) > 10, f"交易日期数量：{len(data)},实际应该大于10天。")
        self.assertTrue(data[0][4] < 1000, f"{data[0][4]=}")
        print(data)
        # index
        data = self.qdb.get_price(
            "000001.XSHG", 20210101, 20210201, '1d', is_dataframe=0)
        self.assertTrue(len(data) > 10, f"交易日期数量：{len(data)},实际应该大于10天。")
        self.assertTrue(data[0][4] > 1000, f"{data[0][4]=}")
        print(f"{data=}\n{type(data)=}")
    ...
    def test_get_price2(self):
        ...
        # index
        data = self.qdb.get_price(
            "000001.12345", 20210101, 20210201, '1d', is_dataframe=0)
        self.assertTrue(len(data) > 10, f"交易日期数量：{len(data)},实际应该大于10天。")
        print(data[-9:])
        ...
        print(data2[-9:])
        self.assertTrue(len(data[0]) == len(data2[0]),
                        f"{data[0]}== {data2[0]}")
        # quantaxis getprice字段：Index(['date', 'open', 'close', 'high', 'low',
        # 'volume', 'datetime'], dtype='object')
        for i in range(len(data)):
            for j in range(len(data[0])):
                if j == 6:
                    # quantaxis、tushare vol字段单位不同，，不能直接比较
                    continue
                self.assertTrue(data[i][j] == data2[i][j],
                                f"{data[i]}== {data2[i]}")

    def test_get_dataFrames(self):
        data = self.qdb.get_dataFrames("000001", 20210101, 20210201, '1d')
        self.assertTrue(len(data[0]) > 10, f"交易日期数量：{len(data)},实际应该大于10天。")
        self.assertTrue((data[0]['open'] < 1000).eq(
            True).all(), f"{data[0]['open']=}")
        print(data)
        # index
        data = self.qdb.get_dataFrames(
            "000001.XSHG", 20210101, 20210201, '1d')
        self.assertTrue(len(data[0]) > 10, f"交易日期数量：{len(data)},实际应该大于10天。")
        self.assertTrue((data[0]['open'] > 1000).eq(
            True).all(), f"{data[0]['open']=}")
        print(data)
    ...
    def test_get_order_book_id_list2(self):
        ...
        self.assertTrue(2000 > len(data) > 1000,
                        f"股票代码数量：{len(data)},实际数量应该大于1000只。")
        # index
        data = self.qdb.get_order_book_id_list("index")
        self.assertTrue(4000 > len(data) > 1000,
                        f"股票代码数量：{len(data)},实际数量({len(data)})应该大于1000只。")
        # all code lise
    ...
    def test_get_trading_dates(self):
        ...
        self.assertTrue(len(data2) > len(data),
                        f"交易日期数量：{len(data)}， {len(data2)},实际天数应该大于前一个交易天数")
    ...
    def test_freq(self):
        print(
            f"freq: '{get_current_freq()}' {get_current_date()}, {get_current_security()}")
        set_current_freq("1d")
        c1 = CLOSE
        lc1 = len(c1.series)
        print(f"day :{lc1}")
        ...
        print(f"week :{lc2}")
        self.assertTrue(
            lc1 > lc2, f"日线数据比周线数据多：{lc1}, {lc2};{get_start_date()} - {get_current_date()}")
    ...
    def test_get_trading_dates3(self):
        ...
        trading_dates = self.qdb.get_trading_dates(
            start=start_date, end=end_date)
        print("请检查本地数据是否完整，再运行本测试。", trading_dates)
        self.assertTrue(
            start_date == trading_dates[0], f"开始日期应该相同：{start_date},{trading_dates[0]}")
        self.assertTrue(
            end_date == trading_dates[-1], f"结束日期应该相同：{end_date},{trading_dates[-1]}")
